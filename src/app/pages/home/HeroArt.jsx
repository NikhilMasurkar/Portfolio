 ...
 * card and a GitHub-activity card, each floating independently. Pure CSS/DOM
 * apart from one real screenshot.
 *
 * The four heatmap shades and three window-dot colours below are the only raw
 * colour literals in this codebase — everything else goes through tokens.
 * They are traffic-light and GitHub-heatmap conventions rather than brand
 * colours, so a token would be inventing meaning that is not there.
 */
 ...
 * server-rendered grid and the client render always agree. Math.random() here
 * would produce a different grid on each side and fail hydration.
 */
function shadeForCell(index) {
  return HEATMAP_SHADES[(index * 5 + 3) % HEATMAP_SHADES.length];
}

export default function HeroArt() {
  return (
    <div aria-hidden="true" className="relative min-h-[520px] max-[900px]:hidden">
