...
Bucket: **`nik-portfolio-new`**, region **`ap-south-1`** (Mumbai). Update
`https://nikhilmasurkar.netlify.app` in the CORS block if the domain changes.
  ...
  "Statement": [
    {
      ...
      "Principal": "*",
      "Action": "s3:GetObject",
      "Resource": "arn:aws:s3:::nik-portfolio-new/public/*"
    }
  ]
}
```
...
|---|---|
| Access key (`AKIA…`) | `AWS_ACCESS_KEY_ID_NEW` |
| Secret access key | `AWS_SECRET_ACCESS_KEY_NEW` |

The `_NEW` suffix is not decoration: Netlify reserves `AWS_ACCESS_KEY_ID`,
`AWS_SECRET_ACCESS_KEY` and `AWS_REGION` for its own build image and will not
accept them as site variables. `AWS_S3_BUCKET` is not reserved, which is why
it alone has no suffix.
...
```
AWS_REGION_NEW=          # ap-south-1 — suffixed, Netlify reserves AWS_REGION
AWS_S3_BUCKET=           # nik-portfolio-new
AWS_ACCESS_KEY_ID_NEW=   # IAM user's key — suffixed, reserved by Netlify
AWS_SECRET_ACCESS_KEY_NEW= # IAM secret — suffixed, reserved by Netlify
S3_PUBLIC_BASE_URL=      # https://nik-portfolio-new.s3.ap-south-1.amazonaws.com
```
